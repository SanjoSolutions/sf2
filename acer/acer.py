...
def q_retrace(R, D, q_i, v, rho_i, nenvs, nsteps, gamma):
    ...
    """
    rho_bar = batch_to_seq(tf.minimum(1.0, rho_i), nenvs,
                           nsteps, True)  # list of len steps, shape [nenvs]
    ...
    for i in range(nsteps - 1, -1, -1):
        check_shape([qret, ds[i], rs[i], rho_bar[i],
                     q_is[i], vs[i]], [[nenvs]] * 6)
...
class Model(object):
    def __init__(self, policy, ob_space, ac_space, nenvs, nsteps, ent_coef, q_coef, gamma, max_grad_norm, lr,
        ...
        with tf.variable_scope('acer_model', reuse=tf.AUTO_REUSE):
            ...
            step_model = policy(
                nbatch=nenvs, nsteps=1, observ_placeholder=step_ob_placeholder, sess=sess)
            train_model = policy(nbatch=nbatch, nsteps=nsteps,
        ...
        with tf.variable_scope("acer_model", custom_getter=custom_getter, reuse=True):
            polyak_model = policy(nbatch=nbatch, nsteps=nsteps,
                                  observ_placeholder=train_ob_placeholder, sess=sess)
        ...
        gain_bc = tf.reduce_sum(logf_bc * tf.stop_gradient(adv_bc * tf.nn.relu(
            1.0 - (c / (rho + eps))) * f), axis=1)  # IMP: This is sum, as expectation wrt f
        ...
        ev = q_explained_variance(tf.reshape(
            q_i, [nenvs, nsteps]), tf.reshape(qret, [nenvs, nsteps]))
        ...
        if trust_region:
            g = tf.gradients(- (loss_policy - ent_coef * entropy)
                             * nsteps * nenvs, f)  # [nenvs * nsteps, nact]
            ...
            grads = [gradient_add(g1, g2, param) for (
                g1, g2, param) in zip(grads_policy, grads_q, params)]
        ...
        run_ops = [_train, loss, loss_q, entropy,
                   loss_policy, loss_f, loss_bc, ev, norm_grads]
        ...
        def train(obs, actions, rewards, dones, mus, states, masks, steps):
            ...
            td_map = {train_model.X: obs, polyak_model.X: obs,
...
class Acer():
    ...
    def call(self, mini_batch, on_policy):
        ...
        if on_policy and (int(steps/runner.nbatch) % self.log_interval == 0):
            ...
            logger.record_tabular("fps", int(
                steps/(time.time() - self.tstart)))
    ...
    def learn(self, mini_batch):
        ...
        for acer.steps in range(0, total_timesteps, nbatch):
            ...
            if save_interval and (acer.steps % save_interval == 0 or acer.steps == 1) and logger.get_dir():
...
def create_acer(network, env, nsteps=20, total_timesteps=int(80e6), q_coef=0.5, ent_coef=0.01,
                max_grad_norm=10, lr=7e-4, lrschedule='linear', rprop_epsilon=1e-5, rprop_alpha=0.99, gamma=0.99,
                save_interval=1000, log_interval=100, buffer_size=50000, replay_ratio=4, replay_start=10000, c=10.0,
    ...
    save_interval:      int, number of updates between saving events (default: 1000)
